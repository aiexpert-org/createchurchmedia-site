...
export const metadata: Metadata = {
  title: "Journal",
  description:
    "Notes on church design, sermon series art, brand consistency, and how design subscriptions actually work for ministry teams.",
          ...
          <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post) => (
              <li key={post.slug}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group block h-full rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-card)] p-7 transition-colors hover:border-[color:var(--color-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--color-accent)]"
                >
        ...
        <Container>
          <div className="rounded-2xl bg-[color:var(--color-ink)] text-[color:var(--color-background)] p-10 lg:p-14 grid lg:grid-cols-[1.4fr_1fr] gap-10 items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 lg:justify-end">
              <ButtonLink
                href={`mailto:${siteConfig.contact.email}?subject=Add me to the journal list`}
                variant="primary"
                size="lg"
                withArrow
              >
